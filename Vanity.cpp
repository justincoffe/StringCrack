...
void VanitySearch::FindKeyGPU_RevDoor(TH_PARAM* ph) {
    ...
    if (!g.SetStringCrackConfig(secp, scConfig)) {
        printf("[SEP7-RD] GPU[%d] Failed to upload config!\n", sliceId);
        ph->isRunning = false;
        return;
    }
    
    // Upload D-table AND C(n,k) table for device-side unranking
    if (!g.ComputeDTable(secp, scConfig)) {
        printf("[SEP7-RD] GPU[%d] Failed to compute D-table!\n", sliceId);
        ph->isRunning = false;
        return;
    }
    
    // Upload G_free tables (needed for initial point construction via window method)
    if (!g.ComputeGfreeTables(secp, scConfig)) {
        printf("[SEP7-RD] GPU[%d] Failed to compute G_free tables!\n", sliceId);
        ph->isRunning = false;
        return;
    }
    ...
    if (n > 64) {
        printf("[SEP7-RD] GPU[%d] ERROR: RevDoor supports up to 64 free bits, got %d\n", sliceId, n);
        ph->isRunning = false;
        return;
    }
    if (maxRadius > n) maxRadius = n;
    ...
    printf("[SEP7-RD] GPU[%d] numWalks=%d, chunk=%d, candidates/launch=%llu\n",
           sliceId, numWalks, chunk_size,
           (unsigned long long)numWalks * chunk_size);
    fflush(stdout);
    ...
    for (int h = minRadius; h <= maxRadius && !endOfSearch; h++) {
        ...
        printf("[SEP7-RD] GPU[%d] h=%d: revdoor positions [%llu, %llu) of %llu total\n",
               sliceId, h,
               (unsigned long long)sliceStart, (unsigned long long)sliceEnd,
               (unsigned long long)layerCombs);
        fflush(stdout);
        ...
        while (pos_offset < sliceEnd && !endOfSearch) {
            ...
            if (!firstBatch) {
                ...
                for (int fi = 0; fi < (int)found.size() && !endOfSearch; fi++) {
                    ITEM it = found[fi];
                    uint32_t hit_walk_id = it.thId;
                    uint32_t hit_step = ((uint32_t)(it.endo & 0x7FFF))
                                      | ((uint32_t)(it.incr & 0x7FFF) << 15);
                    
                    reconstructRevDoorKey(
                        hit_walk_id, hit_step,
                        streamH[prev_s], streamPosBase[prev_s],
                        streamChunkSize[prev_s],
                        it.hash, scConfig, h_combTable, tableK);
                }
                found.clear();
            }
            ...
            // Advance position by actual batch coverage
            pos_offset += (remaining < batchCoverage) ? remaining : batchCoverage;
            totalKeysProcessed += (remaining < batchCoverage) ? remaining : batchCoverage;
            ...
            if (sliceId == 0) {
                ttot = Timer::get_tick() - t0 + t_Paused;
                static double   lastStatsTime_rd = 0.0;
                static uint64_t lastStatsKeys_rd = 0;
                if (ttot - lastStatsTime_rd >= 0.5 || lastStatsTime_rd == 0.0) {
                    double   dt         = ttot - lastStatsTime_rd;
                    uint64_t globalKeys = getGPUCount();
                    uint64_t dk         = (globalKeys >= lastStatsKeys_rd)
                                          ? globalKeys - lastStatsKeys_rd : 0;
                    double spd = (lastStatsTime_rd > 0.0 && dt > 0.0)
                                 ? (double)dk / (dt * 1e6) : 0.0;
                    lastStatsTime_rd = ttot;
                    lastStatsKeys_rd = globalKeys;
                    printf("[SEP7-RD] GLOBAL h=%d | %.1f MK/s | %.2f BKeys | Found: %d     \r",
                           h, spd, (double)globalKeys / 1e9, nbFoundKey);
                    fflush(stdout);
                }
            }
    ...
    if (!firstBatch) {
        int last_s = (g.currentStep - 1) % 2;
        uint32_t nbFound = g.SyncRevDoorBatch(last_s, found);
        for (int fi = 0; fi < (int)found.size() && !endOfSearch; fi++) {
            ITEM it = found[fi];
            uint32_t hit_walk_id = it.thId;
            uint32_t hit_step = ((uint32_t)(it.endo & 0x7FFF))
            ...
            reconstructRevDoorKey(
                hit_walk_id, hit_step,
                streamH[last_s], streamPosBase[last_s],
                streamChunkSize[last_s],
                it.hash, scConfig, h_combTable, tableK);
        }
    }
    ...
    ttot = Timer::get_tick() - t0 + t_Paused;
    double avg_speed = (ttot > 0.0) ? (double)totalKeysProcessed / (ttot * 1e6) : 0.0;
    printf("\n[SEP7-RD] GPU[%d] COMPLETE — %.2f BKeys in %.1fs (%.1f MK/s avg) | Found: %d\n",
           sliceId, (double)totalKeysProcessed / 1e9, ttot, avg_speed, nbFoundKey);
    fflush(stdout);
    
    time_t now_t = time(NULL);
    char* ct = ctime(&now_t);
    printf("[SEP7-RD] GPU[%d] END time: %s", sliceId, ct);
