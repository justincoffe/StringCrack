...
typedef struct {
    ...
    // Windowed precomputation for 4-bit windows (16 windows max, 16 values each)
    int numWindows;  // Number of windows (ceil(numFreeBits/4))
    uint64_t windowGX[16][16][4];  // [window][value][ limb]
    uint64_t windowGY[16][16][4];
